...
# === 1. Captura de parâmetros via URL (para receber ?barcode=XYZ) ===
query_params = st.query_params
if "barcode" in query_params:
    # st.query_params devolve lista de valores; pegamos o primeiro
    st.session_state["codigo"] = query_params["barcode"][0]

# === 2. Criação/garantia de tabela no SQLite ===
criar_tabela()

# === 3. Campo "Crédito disponível" ===
credito_inicial = st.number_input("💰 Crédito disponível", min_value=0.0, value=200.0)

# === 4. Seletor de meses já cadastrados ===
meses = listar_meses()
mes_escolhido = st.selectbox("📆 Escolha o mês", options=meses if meses else ["Nenhum dado"], index=0)

# === 5. Inicialização de session_state para campos do formulário ===
...
with st.form("formulario"):
    # Campo 'Código de barras' — já será preenchido se veio ?barcode=...
    ...
    # === 6.1. Abrir a câmera e iniciar scanner ===
    if abrir_camera:
        escanear_codigo_web()
        # Após a detecção, o navegador será redirecionado para ?barcode=XYZ, 
        # o que fará com que, na próxima renderização, st.session_state["codigo"] seja preenchido.

    # === 6.2. Buscar produto na OpenFoodFacts (pela API externa) ===
    if buscar and codigo_input:
        ...
        if info:
            ...
            st.warning("⚠️ Produto não encontrado na base externa.")

    # === 6.3. Campos de entrada manual (podem ser editados mesmo após busca) ===
    ...
    if adicionar and st.session_state["codigo"]:
        ...
        # Limpa session_state dos campos do formulário
        for campo in ["codigo", "nome", "marca", "fabricante", "categoria"]:
            st.session_state[campo] = ""
        st.rerun()  # força recarregar para limpar o formulário

    # === 6.5. Cadastrar na Open Food Facts (caso queira mesmo sem ter na base) ===
    ...
    # === 6.6. Limpar formulário manualmente ===
...
# === 7. Exibição dos produtos do mês escolhido ===
if mes_escolhido and mes_escolhido != "Nenhum dado":
    ...
    # === 7.2. Botões de Exportar CSV / Excel e Limpar registros do mês ===
...
# === 8. Gráfico comparativo de gastos entre meses ===
