...
# Se vier ?barcode=XYZ na URL, grava em session_state["codigo"] antes de criar a tabela
...
# Garante que as chaves existam em st.session_state
for campo in ["codigo", "nome", "marca", "fabricante", "categoria"]:
    if campo not in st.session_state:
        st.session_state[campo] = ""

with st.form("formulario"):
    # Deixa o próprio text_input usar "codigo" como chave
    ...
    # Se clicar em ler pela câmera, abre o componente e mantém st.session_state["codigo"]
    if abrir_camera:
        escanear_codigo_web()
        # Assim que o JavaScript colocar ?barcode=..., o Streamlit irá recarregar e
        # o bloco de query_params mais acima redefinirá st.session_state["codigo"] automaticamente.

    # Se clicar em “Buscar Produto” e o texto não estiver vazio, faz a requisição
    if buscar and st.session_state["codigo"].strip() != "":
    ...
    # Campos de entrada manual (agora eles usam a session_state para sobrescrever)
    ...
    if adicionar and st.session_state["codigo"].strip() != "":
        ...
        # Não apagamos st.session_state["codigo"] aqui, apenas limpamos os demais campos, se quiser:
        for campo in ["nome", "marca", "fabricante", "categoria", "valor_unitario", "quantidade"]:
            st.session_state[campo] = ""
        st.rerun()

    # Cadastrar na Open Food Facts
    if cadastrar and st.session_state["codigo"].strip() != "" and st.session_state["nome"].strip() != "":
    ...
    # Limpar FORMULÁRIO (opcionalmente podemos zerar todos os campos, inclusive código)
        ...
        if isinstance(linha_selecionada, list) and len(linha_selecionada) >= 1:
            ...
            if "id" not in produto:
                ...
                with st.form("editar_produto"):
                    ...
                    novo_valor = st.number_input("💵 Valor unitário", value=float(produto.get("valor_unitario")), min_value=0.0, key="edit_valor")
                    nova_qtd = st.number_input("🔢 Quantidade", value=int(produto.get("quantidade")), min_value=1, key="edit_quantidade")
