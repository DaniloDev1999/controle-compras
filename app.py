...
# Captura código lido pela câmera via URL e já busca o produto
query_params = st.query_params
if "barcode" in query_params:
    codigo_lido = query_params["barcode"][0]
    st.session_state["codigo"] = codigo_lido
    # Executa busca automaticamente assim que o código chega pela URL
    info_auto = buscar_produto_por_codigo(codigo_lido)
    if info_auto:
        st.session_state["nome"]       = info_auto.get("nome", "")
        st.session_state["marca"]      = info_auto.get("marca", "")
        st.session_state["fabricante"] = info_auto.get("fabricante", "")
        st.session_state["categoria"]  = info_auto.get("categoria", "")
